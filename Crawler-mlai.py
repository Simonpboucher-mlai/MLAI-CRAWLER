        ...
        try:
            if os.path.exists(self.proxy_file):
                with open(self.proxy_file, 'r') as f:
                    ...
                    loaded_proxies = proxy_data.get('proxies', [])
            ...
            if not self.proxies:
                # Default to free proxy list if no proxies are loaded
...
class EnhancedCrawler:
    ...
    async def _handle_request(self, url: str, method: str = 'get', **kwargs) -> Optional[requests.Response]:
...
async def _download_file(self, url: str, category: str) -> Optional[dict]:
        ...
        try:
            response = await self._handle_request(url, stream=True)
            if not response:
                return None
...
async def main():
    ...
    pip install requests beautifulsoup4 PyPDF2 pdf2image pytesseract PyMuPDF Pillow aiohttp
    ...
    crawler = EnhancedCrawler(
        ...
        request_delay=0.2,
        download_files=True,
        proxy_file="proxies.json"  # Optional: provide your proxy list
    )
